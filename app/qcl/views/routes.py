        ...
        if server_session.is_open():
            ...
            try:
                ...
                response = make_response(
                    render_template(
                        "error_internal_server_error.html.j2", message=message
                    ),
                    500,
                )
    return response
...
def internal_server_error(error):
    return (
        render_template(
            "error_internal_server_error.html.j2", message=error.description
        ),
        500,
    )
        ...
        if form.validate():  # form ok
            ...
            if valid:
                ...
                return render_template(
                    "email_verification.html.j2", form=form, error=True
                )
        else:  # form not ok
        ...
        if form.generate.data:  # clicked generate keywords
            ...
            if form.validate():  # data ok
                ...
                try:
                    ...
                    function_id = function.save_function(
                        code, tests, keywords_str, usecase, name, user_id
                    )
                except ValueError:
                    error = "Your source code was rejected"
                    return render_template("classify.html.j2", form=form, error=error)
