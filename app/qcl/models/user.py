...
def new_users_count_total() -> int:
    ...
    query = """
        SELECT COUNT(*)
        FROM account_events
        WHERE event_type='create' AND event_time > :time_filter
        """
...
def new_users_count_ip(remote_ip: str) -> int:
    ...
    query = """
        SELECT COUNT(*)
        FROM account_events
        WHERE event_type='create' AND remote_ip=:remote_ip AND event_time > :time_filter"""
    params = {"time_filter": time_filter, "remote_ip": remote_ip}
    try:
        result = dbrunner.execute(query, params)
    except Exception as e:
        raise RuntimeError(
            "Failed to retrieve amount of recently created users, by remote ip"
        ) from e
    count = result.first()[0]
    assert isinstance(count, int)
    return count
...
def list_users() -> dict:
    ...
    for item in data:
        item["created"] = datetime.fromtimestamp(item["created"]).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        item["locked"] = (
            True
            if item["locked"]
            and item["locked"] > general.get_time_seconds_ago(USER_LOCKOUT_DURATION)
            else False
        )
...
class User:
    ...
    def new(username: str, password: str) -> None:
        ...
        if new_users_ip >= 2:
            raise PermissionError(
                "Refused to create more than 2 users in a day from single IP"
            )
        ...
        query = """
            INSERT INTO users (username, password, verification_code)
            VALUES (:username, :password, :verification_code)
            RETURNING user_id"""
        ...
        user_id = row.user_id
        log.account_creation(
            user_id=user_id, success=True, remote_ip=general.get_remote_ip()
        )
    ...
    def __init__(self, user_id=None, username=None) -> None:
        ...
        self.locked = (
            True
            if row["locked"]
            and row["locked"] > general.get_time_seconds_ago(USER_LOCKOUT_DURATION)
            else False
        )
    ...
    def data_from_id(user_id: UUID) -> dict | None:
        query = """
            SELECT user_id, username, password, verification_code, admin, verified, disabled, locked, created
            FROM users
    ...
    def lock(self) -> None:
        ...
        query = """
            UPDATE users
            SET locked=EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)
            WHERE user_id=:user_id
            """
        params = {"user_id": self.id}
        try:
    ...
    def count_active_sessions(self) -> int:
        query = """
            SELECT COUNT(*)
            FROM sessions
            WHERE user_id=:user_id AND created > :time_filter
            """
