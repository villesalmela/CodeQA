...
def search_keywords(arg: str) -> list[SearchResult]:
    query = """
        SELECT function_id, name, keywords
        FROM functions
        WHERE keywords ILIKE :arg
        """
...
def search_usecase(arg: str) -> list[SearchResult]:
    query = """
        SELECT function_id, name, usecase
        FROM functions
        WHERE usecase ILIKE :arg
        """
...
def search_code(arg: str) -> list[SearchResult]:
    query = """
        SELECT function_id, name, code
        FROM functions
        WHERE code ILIKE :arg
        """
    target_field = "code"
...
def _search(arg: str, query: str, target_field: str) -> list[SearchResult]:
    ...
    for row in rows:
        out.append(
            SearchResult(
                result_type=target_field,
                function_id=row.function_id,
                function_name=row.name,
                context=getattr(row, target_field),
                match=arg_orig,
            )
        )
    return out
